...
let botoMostrarItems = new Boto(
  "Torna a mostrar els primers items",
  "boto-mostrar-items"
); // Creem instàncies dels botons necessaris

let inputCerca = new Input(
  "InputCerca",
  "text",
  "",
  "Introdueix el nom d'item"
); // Creem instància de l'input de cerca
...
document.addEventListener("DOMContentLoaded", () => {
  // Esperem que el DOM estigui llest i renderitzar
  ...
  contenedor.addEventListener("click", (event) => {
    // Afegim funcio al div amb id "divs"
    if (event.target.id && event.target.id.startsWith("boto-eliminar")) {
      // Si l'element té id i comença per "boto-eliminar"
  ...
  botoCerca.addEventListener("click", () => {
    ...
    dades.forEach((item) => {
      ...
      if (nom === text) {
        ...
        // Si el nom comença amb el text
        itemsComencaAmb.push(item);
      } else if (nom.includes(text) || descripcio.includes(text)) {
        // Si el nom o la descripció contenen el text
        itemsConte.add(item); // Afegim l'item al Set d'items que contenen el text
      }
    });

    let trobat = false; // Variable per controlar si s'ha trobat algun item
    
    // Mostrar alerts segons els resultats de la cerca
    if (itemsExactes.length > 0 && trobat === false) {
      alert("S'ha trobat l'item");
      trobat = true;
    }
    if (itemsComencaAmb.length > 0 && trobat === false) {
      alert("S'ha trobat un item que comença amb el text");
      trobat = true;
    }
    if (itemsConte.size > 0 && trobat === false) {
      alert(
        "S'ha trobat un item que conte el text a dins del nom o la descripció"
      );
      trobat = true;
    }
  ...
  boto10mes.addEventListener("click", () => {
    ...
    if (dadesPerMostrar.length === 0) {
      // Si no hi ha més dades per mostrar
  ...
  botoMostrarItems.addEventListener("click", () => {
    // Afegim funcionalitat al botoMostrarItems
    let dades = JSON.parse(localStorage.getItem("items")) || [];
