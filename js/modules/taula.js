...
class Taula extends ComponentHTML {
  ...
  static crearTaulaHTML(capcaleres, dada) {
    ...
    // Afegir capçaleres
    capcaleres.forEach((capcalera) => {// Recorrer les capcaleres
    ...
    // Afegir dades
    dada.forEach((fila) => {// Recorrem les dades
      taulaHTML += "<tr>";// Obrim una fila
      fila.forEach((cell) => {// Recorrem les cel·les de la fila
  ...
  carregarDades(dadesFiltrades = null) {// Carregar les dades amb les dades filtrades per defecte a null
  ...
  if (dades.length > 10) {// Si hi ha més de 10 items a les dades
  ...
  let files = dades.map((dada) => [// Recorrem les dades i retornem un array amb les dades de cada item
    dada.nom,
    dada.descripcio,
    dada.dataCreacio || "-",
